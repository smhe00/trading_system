...
class GatewayReadOnlyTests(unittest.TestCase):
    ...
    def test_connect_failure_clears_client(self):
        client = make_client()
...
class ClientAccountSelectionTests(unittest.TestCase):
    """QmtClient auto-selects the unique STOCK account; no hard-coded id."""
    ...
    def test_rejects_none_empty_or_multiple(self):
        for infos in (None, [], [NS(account_type=2, account_id="1"), NS(account_type=2, account_id="2")]):
            with self.subTest(infos=infos):
                client = self.make_client(infos)
                with self.assertRaises(RuntimeError):
                    client._select_account_id()

    def test_rejects_non_stock_account(self):
        client = self.make_client([NS(account_type=3, account_id="12345678")])
        with self.assertRaises(RuntimeError):
            client._select_account_id()
